    ...
    EdgeProp: Default + Clone,
{
    ...
    pub fn get_v_prop(&mut self, v: i32) -> Option<&VertexProp> {
        self.v_map.get(&v)
    }

    pub fn get_e_prop(&mut self, e: (i32, i32)) -> Option<&EdgeProp> {
        self.e_map.get(&e)
    }

    pub fn contains_v(&mut self, v: i32) -> bool {
        self.v.contains(&v)
    }
}
...
mod tests {
    ...
    fn vertex_basic_operations() {
        ...
        adjlist.remove_vertex(1);
        assert_eq!(adjlist.v.len(), 1);
    }
    ...
    fn vertex_property_tests() {
        ...
        adjlist.add_vertex_prop(0, vp1);
        assert_eq!(id0, adjlist.get_v_prop(1).unwrap().id);
        assert_ne!(id1, adjlist.get_v_prop(1).unwrap().id);
        assert_eq!(color0, adjlist.get_v_prop(1).unwrap().color);
        assert_ne!(color1, adjlist.get_v_prop(1).unwrap().color);
    }
